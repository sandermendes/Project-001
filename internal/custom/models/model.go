...
func (m *Plugin) generateFields(cfg *config.Config, schemaType *ast.Definition) ([]*Field, error) {
	...
	for _, field := range schemaType.Fields {
		...
		// Commented for not generate the "*" character on variable types,
		// needs to confirm
		...
		f := &Field{
			...
			Gorm:        getStructOrmFromField(field),
		}
...
func getStructOrmFromField(field *ast.FieldDefinition) string {
	directive := field.Directives.ForName("DatabaseField")
	if directive != nil {
		// Check for the fieldName Gorm attribute
		argFieldName := directive.Arguments.ForName("fieldName")
		if argFieldName != nil {
			return fmt.Sprintf(`gorm:"column:%s"`, argFieldName.Value.Raw)
		}
		// Check for the custom Gorm attribute
		argCustom := directive.Arguments.ForName("custom")
		if argCustom != nil {
			return fmt.Sprintf(`gorm:"%s"`, argCustom.Value.Raw)
		}
		return fmt.Sprintf(`gorm:"column:%s"`, field.Name)
	}
	return ""
}
